#!/usr/bin/env python3
"""
Script para visualizar las etapas del proceso de justificación y división silábica
con capturas que muestren los espaciados delimitados.
"""
...
def dibujar_linea_con_espaciado_visualizado(c, texto, x, y, width, font_size, etapa, mostrar_espaciado=True):
    """
    ...
    Args:
        ...
        etapa: Etapa del proceso (1, 2, 3)
        mostrar_espaciado: Si mostrar líneas de espaciado
    """
    ...
    for i, palabra in enumerate(palabras):
        ...
        if i < len(palabras) - 1 and mostrar_espaciado:
            x_espacio = x_pos + pdfmetrics.stringWidth(palabra, GARAMOND_REGULAR_NAME, font_size)
        ...
        x_pos += pdfmetrics.stringWidth(palabra, GARAMOND_REGULAR_NAME, font_size) + espaciado
...
def generar_capturas_etapas():
    ...
    dibujar_linea_con_espaciado_visualizado(c, linea_original, x_inicio, y_etapa2 - 50, max_width, font_size, 2)
    ...
    dibujar_linea_con_espaciado_visualizado(c, linea_dividida, x_inicio, y_etapa3 - 50, max_width, font_size, 3)
    ...
    y_info -= 15
    c.drawString(50, y_info, f"• Espacio ideal (1x): {pdfmetrics.stringWidth(' ', GARAMOND_REGULAR_NAME, font_size):.2f}pt")
    y_info -= 15
    c.drawString(50, y_info, f"• Límite máximo (1.5x): {pdfmetrics.stringWidth(' ', GARAMOND_REGULAR_NAME, font_size) * ESPACIADO_MAXIMO_MULTIPLICADOR:.2f}pt")
    y_info -= 15
    c.drawString(50, y_info, f"• Ancho total palabras original: {sum(pdfmetrics.stringWidth(p, GARAMOND_REGULAR_NAME, font_size) for p in linea_original.split()):.2f}pt")
    y_info -= 15
    c.drawString(50, y_info, f"• Ancho total palabras divididas: {sum(pdfmetrics.stringWidth(p, GARAMOND_REGULAR_NAME, font_size) for p in linea_dividida.split()):.2f}pt")
